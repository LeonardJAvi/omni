...
class Plan < ActiveRecord::Base
  ...
  validates_presence_of :title, :url, :destination_id, :destination_ids, :banner, :type_days, :days
  ...
  def destinos_names
    destinos_planes = self.destinations.map{ |destino|
      if destino.name.include?(' ')
        destino.name.split(' ').join('-')
      else
        destino.name
      end
    }.flatten.uniq.map { |destino| destino.downcase.tr('áéíóúñÁÉÍÓÚÑ', 'aeiounAEIOUN')}

    destinos_planes = destinos_planes.join(" ")
  end
